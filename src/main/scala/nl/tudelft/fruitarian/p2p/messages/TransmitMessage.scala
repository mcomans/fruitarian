...
case class TransmitMessage(from: Address, to: Address, message: List[Byte]) extends FruitarianMessage(MessageHeader(TransmitMessage.MessageType, from, to)) {
...
case object TransmitMessage {
	...
	def fromHeaderAndBody(header: MessageHeader, body: String): FruitarianMessage = {
		TransmitMessage(header.from, header.to, read[List[Byte]](body))
	}
}
